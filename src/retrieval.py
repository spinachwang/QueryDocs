...
class BM25Retriever:
    ...
    def retrieve_by_company_name(self, company_name: str, query: str, top_n: int = 3, return_parent_pages: bool = False) -> List[Dict]:
        ...
        # 加载对应的BM25索引，文件名用 source + MD5（与 BM25Ingestor 保持一致）
        source = document['metainfo']['source']
        safe_name = f"doc_{hashlib.md5(source.encode('utf-8')).hexdigest()[:8]}"
        bm25_path = self.bm25_db_dir / f"{safe_name}.pkl"
...
class VectorRetriever:
    ...
    def _load_dbs(self):
        ...
        for document_path in all_documents_paths:
            try:
                ...
                continue
            # 用 source + MD5 查找 faiss 文件（与 VectorDBIngestor 保持一致）
            source = document.get('metainfo', {}).get('source', None)
            if not source:
                _log.warning(f"No source found in metainfo for document {document_path.name}")
                continue
            safe_name = f"doc_{hashlib.md5(source.encode('utf-8')).hexdigest()[:8]}"
            faiss_path = self.vector_db_dir / f"{safe_name}.faiss"
            if not faiss_path.exists():
                _log.warning(f"No matching vector DB found for document {document_path.name} (source={source})")
                continue
            try:
    ...
    def retrieve_by_company_name(self, company_name: str, query: str, llm_reranking_sample_size: int = None, top_n: int = 3, return_parent_pages: bool = False) -> List[Tuple[str, float]]:
        ...
        if target_report is None:
            ...
            raise ValueError(f"No report found with '{company_name}' company name.")
        # 用 source + MD5 查找 faiss 文件（与 _load_dbs 保持一致）
        source = target_report["document"]["metainfo"].get("source")
        if not source:
            raise ValueError(f"No source found in metainfo for company '{company_name}'")
        safe_name = f"doc_{hashlib.md5(source.encode('utf-8')).hexdigest()[:8]}"
        faiss_path = self.vector_db_dir / f"{safe_name}.faiss"
        if not faiss_path.exists():
            raise ValueError(f"No vector DB found for '{company_name}' (source: {source})")
...
class HybridRetriever:
    def __init__(self, vector_db_dir: Path, documents_dir: Path, rerank_provider: str = "dashscope"):
