...
class APIProcessor:
    def __init__(self, provider: Literal["openai", "ibm", "gemini", "dashscope", "minimax"] ="dashscope"):
        ...
        if self.provider == "openai":
            ...
            self.processor = BaseMiniMaxProcessor()
    ...
    def get_answer_from_rag_context(self, question, rag_context, schema, model):
        ...
        if not isinstance(answer_dict, dict) or 'step_by_step_analysis' not in answer_dict or answer_dict.get('step_by_step_analysis') in (None, ''):
            if isinstance(answer_dict, dict) and 'final_answer' in answer_dict:
                ...
                if isinstance(final_answer_content, str):
                    ...
                    # 方法1：如果直接以 { 开头，直接解析
                    if final_answer_content.strip().startswith('{'):
                        try:
                            answer_dict = json.loads(final_answer_content)
                        except json.JSONDecodeError:
                            pass

                    # 方法2：移除 [Thinking]...[/Thinking] 块，然后解析剩余的JSON
                    if not answer_dict.get('step_by_step_analysis'):
                        cleaned_content = re.sub(r'\[Thinking\].*?\[/Thinking\]', '', final_answer_content, flags=re.DOTALL).strip()
                    ...
                    # 方法3：尝试从 ```json ... ``` 中提取JSON
                    if not answer_dict.get('step_by_step_analysis'):
                        json_match = re.search(r'```json\s*(\{[\s\S]*?\})\s*```', final_answer_content)
                        if json_match:
                            try:
                                answer_dict = json.loads(json_match.group(1))
                            except json.JSONDecodeError:
                                repaired = repair_json(json_match.group(1))
                                answer_dict = json.loads(repaired)
                ...
                if not answer_dict.get('step_by_step_analysis'):
                    answer_dict = {
                        ...
                        "final_answer": final_answer_content if isinstance(final_answer_content, str) else "N/A"
                    }
            else:
                answer_dict = {
...
# MiniMax基础处理器，支持MiniMax-M2.7等模型（Anthropic API兼容）
class BaseMiniMaxProcessor:
    def __init__(self):
        load_dotenv()
    ...
    def send_message(
        ...
        **kwargs
        ...
        if system_content:
            messages.append({"role": "system", "content": system_content})
        if human_content:
            messages.append({"role": "user", "content": [{"type": "text", "text": human_content}]})
        ...
        # is_structured 时尝试启用 thinking（结构化输出依赖模型能力）
        if is_structured:
            params["thinking"] = {"type": "enabled"}

        completion = self.llm.messages.create(**params)
        ...
        for block in content_blocks:
            if block.type == "text":
                ...
                answer_text += f"[Thinking]\n{block.thinking}\n[/Thinking]\n"

        self.response_data = {
            "model": completion.model,
            "input_tokens": completion.usage.input_tokens if hasattr(completion.usage, "input_tokens") else None,
            "output_tokens": completion.usage.output_tokens if hasattr(completion.usage, "output_tokens") else None
        }
        ...
        # 非结构化或解析失败
        return {"final_answer": answer_text, "step_by_step_analysis": "", "reasoning_summary": "", "relevant_pages": []}
