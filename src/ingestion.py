...
class BM25Ingestor:
    ...
    def process_reports(self, all_reports_dir: Path, output_dir: Path):
        ...
        for report_path in tqdm(all_report_paths, desc="Processing reports for BM25"):
            ...
            # 用 source + MD5 命名（与 VectorDBIngestor 保持一致）
            source = report_data["metainfo"]["source"]
...
class VectorDBIngestor:
    ...
    def _to_safe_filename(self, name: str) -> str:
        """Convert Chinese name to safe filename using hash"""
        import hashlib
    ...
    def _get_embeddings(self, text: Union[str, List[str]], model: str = "text-embedding-v1") -> List[float]:
        ...
        for i in range(0, len(text_chunks), MAX_BATCH_SIZE):
            ...
            # with open(LOG_FILE, 'a', encoding='utf-8') as f:
            #     f.write('resp='+str(resp)+'\n')
            if resp and 'output' in resp and resp['output'] and 'embeddings' in resp['output']:
                print('11111111')
            ...
            elif resp and 'output' in resp and resp['output'] and 'embedding' in resp['output']:
    ...
    def process_reports(self, all_reports_dir: Path, output_dir: Path):
        ...
        for report_path in tqdm(all_report_paths, desc="Processing reports for FAISS"):
            ...
            # 用 metainfo['source'] 作为 faiss 文件名
            source = report_data["metainfo"].get("source", "")
            if not source:
                raise ValueError(f"分块报告 {report_path} 缺少 source 字段，无法保存 faiss 文件！")
            safe_name = self._to_safe_filename(source)
            faiss_file_path = output_dir / f"{safe_name}.faiss"
