...
class PipelineConfig:
    def __init__(self, root_path: Path, subset_name: str = "subset.csv", questions_file_name: str = "questions.json", pdf_reports_dir_name: str = "pdf_reports", serialized: bool = False, config_suffix: str = "", run_config=None):
        # 路径配置，支持不同流程和数据目录
        ...
        self.vector_db_dir = self.databases_path / "vector_dbs" if not run_config.vector_db_dir_override else Path(run_config.vector_db_dir_override)
        self.documents_dir = self.debug_data_path / "chunked_reports" if not run_config.documents_dir_override else Path(run_config.documents_dir_override)
...
class RunConfig:
    ...
    config_suffix: str = ""
    # 可选：覆盖默认路径
    vector_db_dir_override: str = ""
    documents_dir_override: str = ""
